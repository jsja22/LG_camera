...
train_csv = pd.read_csv('C:/LG_CAMERA/train.csv')
test_csv = pd.read_csv('C:/LG_CAMERA/test.csv')

print(train_csv.head())

train_all_input_files = 'C:/LG_CAMERA/train_input_img/'+train_csv['input_img']
train_all_label_files = 'C:/LG_CAMERA/train_label_img/'+train_csv['label_img']

print(len(train_all_input_files), len(train_all_label_files)) #622 622
...
val_inp_files = glob(f'C:/LG_CAMERA/val_input_img_{img_size}/*.npy')
val_targ_files = glob(f'C:/LG_CAMERA/val_label_img_{img_size}/*.npy')

train_inp_files, train_targ_files = shuffle(train_inp_files, train_targ_files, random_state=42)

print(len(train_inp_files), len(val_inp_files), len(train_targ_files), len(val_targ_files))
...
callbacks_list = [
    tf.keras.callbacks.ModelCheckpoint(
        filepath = 'C:/LG_CAMERA/h5/baseline_model.h5',
        monitor='val_loss',
        save_best_only=True
    )
]
...
for i, (input_path, label_path) in enumerate(zip(val_input_files[:5], val_label_files[:5])):
    ...
    test_input_files = 'C:/LG_CAMERA/test_input_img/'+test_csv['input_img']
